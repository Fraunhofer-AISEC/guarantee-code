...
/* [Alina]
 * The intel project sgx-ra-tls first used an older wolfssl version, which needed to be compiled
 * with gcc5. We updated the project to use an newer version of wolfssl, which can be built
 * with gcc9 and older (most likely also newer ones). Because of this update the code of intel 
 * here would throw some warnings regarding implicit type casting, espectially from unsigned 
 * integers to signed integers. We fixed these warnings by adding the explicit cast. These casts 
 * can be a source of errors but for our research this fix was sufficient.
 */
...
)
{
    ...
    char *field_end = memmem(field_begin,
                             header_len - (uint64_t)(field_begin - header),
                             http_line_break,
                             strlen(http_line_break));
    size_t field_len = (uint64_t) (field_end - field_begin);
    ...
    uint32_t cert_len = (uint32_t)(cert_end - cert_begin) + (uint32_t)strlen(pem_marker_end);
    ...
    assert(cert_end != NULL);
    cert_len = (uint32_t) (cert_end - (cert_begin + strlen(pem_marker_end)));
...
)
{
    ...
    char* sig_end = memmem(sig_begin,
                           (size_t) ((int64_t)header_len - (sig_begin - header)),
                           "\r\n",
                           strlen("\r\n"));
    assert(sig_end);
